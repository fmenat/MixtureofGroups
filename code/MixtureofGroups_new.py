...
class GroupMixtureOpt_Repeat(object): #Repeat--before was global
    def __init__(self,input_dim,Kl,T,M=2,epochs=1,optimizer='adam',pre_init=10,dtype_op='float32'): 
        if type(input_dim) != tuple:
            input_dim = (input_dim,)
        self.input_dim = input_dim
        self.Kl = Kl #number of classes of the problem
        ...
        self.compile=False
        self.seted_alphainit = False
        self.lambda_random = False #maybe here not...
    ...
    def set_alpha(self,alpha_init):
        ...
        self.seted_alphainit = True
    def get_qestimation(self):
        """Get Q estimation param, this is Q_it(g,z) = p(g,z|xi,a_t,r_it)"""
        return self.Qit_mgamma.copy()
    ...
    def init_E(self,X,r):
        ...
        #-------> init q_ij      
        self.Qit_mgamma = self.alpha_init[None,:,:,None]*self.mv_probs_j[:,None,None,:]#*Zijm #o sacar primer "None"
        
        #-------> init betas
        self.betas = np.zeros((self.M,self.Kl,self.Kl),dtype=self.DTYPE_OP) 

        #-------> init alphas
        self.alphas = np.zeros((self.M,self.T),dtype=self.DTYPE_OP)
    ...
    def E_step(self,X,y_ann,predictions=[]):
        ...
        p_new = np.clip(predictions[:,None,None,:] , self.Keps,1.) #safe logarithmn
        a_new = np.clip(self.alphas[None,:,:,None] , self.Keps,1.) #safe logarithmn
        
        #calculate annotators confusion/credibility
        a_igamma = np.tensordot(y_ann, np.log(self.betas + self.Keps),axes=[[1,2],[0,2]]) #revisar este producto...
        a_igamma = a_igamma.astype(self.DTYPE_OP) #ver eso

        self.Qit_mgamma = np.log(p_new) + np.log(a_new) + a_igamma #logarithm version
        
        self.aux_for_like = (self.Qit_mgamma.sum(axis=-1)).sum(axis=-1) #p(y=j|x) --marginalized

        self.Qit_mgamma = np.exp(aux-aux.max(axis=-1,keepdims=True)) #return to actually values
        self.Qit_mgamma = self.Qit_mgamma/self.Qit_mgamma.sum(axis=-1,keepdims=True) #normalize q
        
    def M_step(self,X,y_ann): 
        """ Realize the M step"""
        #-------> base model
        Qit_gamma = self.Qit_gamma.sum(axis=-2) #qit(gamma)
        Qi_gamma = Qit_gamma.sum(axis=1)
        if "sklearn" in self.type:#train to learn p(z|x)
            self.base_model.fit(X, Qi_gamma.argmax(axis=1) ) 
        else:
            self.base_model.fit(X,Qi_gamma,batch_size=self.batch_size,epochs=self.epochs,verbose=0) 
    
        #-------> alpha 
        Qit_m = self.Qit_mgamma.sum(axis=-1) #qit(m)
        Qt_m = Qit_m.sum(axis=0)
        self.alphas = Qt_m
        self.alphas = self.alphas/self.alphas.sum(axis=-1,keepdims=True) #p(g) -- normalize
        
        #-------> beta
        self.betas[:,:,:] = np.tensordot(self.Qit_mgamma,y_ann, axes=[[0,1],[0,1]]) # ~p(yo=j|g,z)       
        #for j_ob in range(self.Kl):
        #    self.betas[:,:,j_ob] = np.tensordot(self.Qit_mgamma[:,j_ob,:,:],r[:,j_ob], axes=[[0,1],[0,1]]) # ~p(yo=j|g,z)              
        if self.priors:
            self.betas += self.Mpriors #priors has to be shape: (M,Kl,Kl)--read define-prior functio
        self.betas = self.betas/self.betas.sum(axis=-1,keepdims=True) #normalize (=p(yo|g,z))
    ...
    def compute_logL(self,r,predictions):
        """ Compute the log-likelihood of the optimization schedule"""
        return np.sum( np.sum( np.log(self.aux_for_like+self.Keps) ,axis=-1),axis=-1)#safe logarithm

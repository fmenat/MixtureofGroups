...
from malr.supervised import LogisticRegression

class MALogisticRegressionMVHard(Classifier):
	"""Majority Voting Logistic Regression"""
	...
	def majority_voting(self, dataset):
		...
		for i in xrange(dataset.num_instances):
			id = dataset.ids[i][0]
			if not votes.has_key(id):
				votes[id] = np.zeros(self.num_classes)
			votes[id][dataset.targets[i]] += 1
		estimated_ground_truth = []

		# compute most voted
		for i in xrange(dataset.num_instances):
			_id = dataset.ids[i][0]
			label = np.argmax(votes[_id])
			new_instance = np.zeros(self.num_classes)
			new_instance[label] = 1.0
			estimated_ground_truth.append(new_instance)
			
		estimated_ground_truth = np.array(estimated_ground_truth)
		print "estimated_ground_truth:", estimated_ground_truth
		...
		if self.ground_truth != None:
			...
			for i in xrange(self.ground_truth.num_instances):
				...
				if self.ground_truth.targets[i] == np.argmax(votes[_id]):
					acc += 1.0
				count += 1.0
			print "\tMajority Voting accuracy:", (acc / count)*100.0
		return estimated_ground_truth

	def train(self):
		print "Training Majority Voting Logistic Regression model..."
		...
		self.t_k = self.estimated_ground_truth
